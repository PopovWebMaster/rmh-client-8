...
export class RowMartixHeaderDayNamesClass extends RowClass {
    ...
    Create(){
        ...
        for( let i = 0; i < this.dayNamesList.length; i++ ){
            arr_F14_AL14.push( this.GetCell( this.dayNamesList[ i ] ) );
            count++;
        };

        for( let i = count; i < DAY_LENGTH; i++ ){
            arr_F14_AL14.push( this.GetCell( '' ) );
        };

        row = [ ...arr_A14_E14, ...arr_F14_AL14, ...arr_AM14_AN14 ];
        ...
        this.AddRange( `AM${this.rowNumber}:AM${this.rowNumber + 1 }` );
